...
function ticket(options){
    ...
    return function(req,res,next){
        ...
        const request_ticket = bent( cas_uri, {"Accept": "application/xml"});
        // eventually rewrite this whole library to use async/await
        // for now, just use then

        request_ticket().then( (error,response,body)=>{
            console.log('back from bent request')
            if (!error && response.statusCode == 200) {
                var xmlparser = require('./xml_parser').make_xml_parser(req,res,next)
                xmlparser(body)
            }else{
