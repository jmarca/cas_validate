              ...
              parseString(body, function(err, result) {
                ...
                if (!err) {
                  ...
                  if (authResult['cas:authenticationSuccess']) {
                    ...
                    success['cas:'+(options.attribute_name||'attributes')].forEach(function(attr) {
                      var previous = req.session.attributes[attr.$.name];
                      if (previous) {
                          if (typeof previous === 'string')
                            req.session.attributes[attr.$.name] = [previous];
                        req.session.attributes[attr.$.name].push(attr.$.value);
                      }
                      else
                        req.session.attributes[attr.$.name] = attr.$.value;
                    });
                  }
                  else {
