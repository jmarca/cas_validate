...
function make_sax_parser(req,res,next){
    ...
    if(req.session.name){
                // FIXME: crindt: the append here is leading to repeated strings for 
                // req.session.name when CAS gateway is used.  e.g., 
                //    first call: req.session.name=username
                //   second call: req.session.name=usernameusername
                //    third call: req.session.name=usernameusernameusername
