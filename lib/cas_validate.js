        ...
        request({uri:cas_uri}, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                if(/cas:authenticationSuccess/.exec(body)){
                    ...
                    var failed = xmlDoc.get('/cas:serviceResponse/cas:authenticationFailure',{"cas": "http://www.yale.edu/tp/cas"})
                    console.log({name:failed.name(),
                                 code:failed.attr('code').value(),
                                })
                    if(failed.attr('code').value()=='INVALID_TICKET'){
