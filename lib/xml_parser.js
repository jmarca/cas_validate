...
const  logger = require('./logger')('cas_validate::sax_parser')
const set_session = require('./ticket_store').set_session
const _ = require('underscore')

function make_xml_parser(req,res,next){
    logger.debug('in make xml parser')
    logger.debug('res is defined? '+(res != undefined))
    var errors
    ...
    function parse(doc,encoding){
        ...
        if(req.session.st){
            ...
            logger.debug('parsed xml doc, did not get user info.  probably a bad ticket')
            // handling an API change.   url isn't
            // passed through cleanly anymore, as the "mount point" is
            // stripped.  And url.parse is deprecated.  So...

            // express and connect now store original url in req.originalUrl
            const myURL = new URL(req.originalUrl, 'https://example.org/')

            logger.debug('redirect to '+ myURL.pathname + ', second pass should skip CAS check and fall through')
            res.writeHead(307, { 'location': myURL.pathname })
            res.end()
        }
        return null
    }
